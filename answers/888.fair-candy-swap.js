...
var fairCandySwap = function(aliceSizes, bobSizes) {
  ...
  for (let i = 0; i < aliceSizes.length; i++) {
    aliceCandySum += aliceSizes[i]
  }

  for (let i = 0; i < bobSizes.length; i++) {
    bobCandySum += bobSizes[i]
  }

  const half = (aliceCandySum + bobCandySum) / 2
  const offset = half - aliceCandySum

  const wishlist = Object.create(null)

  for (let i = 0; i < aliceSizes.length; i++) {
    const count = aliceSizes[i]

    wishlist[count + offset] = count
  }

  for (let i = 0; i < bobSizes.length; i++) {
    const count = bobSizes[i]

    if (wishlist[count] !== undefined) {
      return [wishlist[count], count]
    }
  }
};
// @lc code=end
